  ...
  async connect(onConnected, onError) {
    if (this.assistInstance === null) {
      ...
      let walletSelectConfig = Onboard.modules.select.defaults({
        portisInit: { apiKey: this.portisAPIKey },
        walletConnectInit: { infuraKey: this.infuraKey },
        networkId: 1
      });
      ...
      this.assistInstance = Onboard.init(bncAssistConfig);
    }
    ...
    if (
      ...
      let ready = await this.assistInstance.walletReady();
